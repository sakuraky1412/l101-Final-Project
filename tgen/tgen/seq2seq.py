...
class Seq2SeqBase(SentencePlanner):
    ...
    def _beam_search(self, enc_inputs, da):
        ...
        self.sample_top_k = self.beam_size

        if self.sample_top_k > 1:
            # best_path = self._sample_path(paths[:self.sample_top_k])
            best_path = paths[:self.sample_top_k]
        else:
            best_path = paths[0]

        with open("prob.txt", 'a+') as text_file:
            best_path_dec_inputs = []
            averages = []
            for path in best_path:
                path_dec_states = path.dec_states
                lstm_h = []
                for lstm_state_tuple in path_dec_states:
                    c, h = lstm_state_tuple
                    lstm_h.append(h)
                average_lstm_h = np.average(np.array(lstm_h), axis=0)
                averages.append(average_lstm_h)
                path_dec_inputs = path.dec_inputs
                path_dec_inputs.append(np.asarray([58]))
                path_dec_inputs.append(np.asarray([85]))
                path_dec_inputs.append(np.asarray([34]))
                best_path_dec_inputs.extend(path_dec_inputs)
                text_file.write(("%f\t" % path.logprob) + "\n")
            text_file.write("\n")

        with open("lstm.txt", 'a+') as text_file:
            text_file.write(("%d\t" % self.lstm_states(best_path, averages)) + "\n")

        # return just the best path (as token IDs)
        # return np.array(best_path.dec_inputs)
        return np.array(best_path_dec_inputs)

    def lstm_states(self, best_path, averages):
        lstm_scores = []
        for i in range(len(best_path)):
            cur_prob = best_path[i].logprob
            cur_aver = averages[i]
            lstm_score = 0
            for j in range(len(best_path)):
                other_prob = best_path[j].logprob
                other_aver = averages[j]
                lstm_score += other_prob * cosine_similarity(cur_aver, other_aver)
            lstm_scores.append([lstm_score, i])
        lstm_scores.sort(key=lambda tup: tup[0])
        return lstm_scores[0][1]
